#!/bin/bash
DEPLOY_PATH='/home/yc-user/reddit'
...
else "WARN: Repo monolith wasn't cloned" >> /tmp/log_deploy.log exit 1;
fi
cd $DEPLOY_PATH && bundle install && puma -d > /tmp/puma_out.err
cat /tmp/puma_out.err | grep "Puma starting in single mode..."
if [ $? -eq 0 ]; then
echo "Puma is UP and running" >> /tmp/log_deploy.log
else "WARN: Puma wasn't started" >> /tmp/log_deploy.log exit 1;
fi
exit 0
